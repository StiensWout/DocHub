# DocHub Documentation

> **Start here**: See [Documentation Index](INDEX.md) for organized navigation

Welcome to the DocHub documentation. This is your central hub for all documentation.

---

## 🚀 Quick Start

**New to DocHub?**

1. Read the [Current Status](STATUS.md)
2. Follow the [Installation Guide](GETTING_STARTED/INSTALLATION.md)
3. Set up [WorkOS Authentication](SETUP/WORKOS.md)
4. Configure [Admin Access](ADMIN_SETUP.md)

---

## 📚 Documentation Structure

### Main Documentation
- **[INDEX.md](INDEX.md)** ⭐ - **Main entry point - Start here!**
- **[STATUS.md](STATUS.md)** - Current project status
- **[ROADMAP.md](ROADMAP.md)** - Feature roadmap
- **[CHANGELOG.md](CHANGELOG.md)** - Version history

### Getting Started
- **[Installation](GETTING_STARTED/INSTALLATION.md)** - Local setup
- **[Configuration](GETTING_STARTED/CONFIGURATION.md)** - Environment setup
- **[Supabase Setup](GETTING_STARTED/SUPABASE_SETUP.md)** - Database setup

### Setup & Configuration
- **[WorkOS Setup](SETUP/WORKOS.md)** - Authentication provider
- **[SSO Configuration](SETUP/SSO.md)** - Single Sign-On
- **[Provider Migration](SETUP/PROVIDER_MIGRATION.md)** - Switch SSO providers

### User Guides
- **[Rich Text Editor](GUIDES/rich-text-editor.md)** - Using the editor

### Architecture
- **[Overview](ARCHITECTURE/OVERVIEW.md)** - System architecture
- **[Components](ARCHITECTURE/COMPONENTS.md)** - Component structure
- **[Database](ARCHITECTURE/DATABASE.md)** - Database schema

### Development
- **[Development Guide](DEVELOPMENT/GUIDE.md)** - Contributing
- **[Testing Guide](DEVELOPMENT/TESTING.md)** - Testing practices

### Support
- **[Troubleshooting](TROUBLESHOOTING.md)** - Common issues
- **[FAQ](FAQ.md)** - Frequently asked questions

### Project Management
- **Bug Tracking:** All bugs are tracked in [GitHub Issues](https://github.com/StiensWout/DocHub/issues)
- **Task Management:** Tasks are tracked in [GitHub Projects](https://github.com/StiensWout/DocHub/projects)
- **Historical Project Management:** Archived in `docs/archive/bugs/` (migrated to GitHub)

---

## 🎯 Find What You Need

| I want to... | See |
|--------------|-----|
| **Understand the project** | [Status](STATUS.md) or [Index](INDEX.md) |
| **Set up locally** | [Installation](GETTING_STARTED/INSTALLATION.md) |
| **Configure auth** | [WorkOS Setup](SETUP/WORKOS.md) |
| **View features** | Check [GitHub Issues](https://github.com/StiensWout/DocHub/issues) - features are tracked in git |
| **See what's planned** | [Roadmap](ROADMAP.md) |
| **Contribute code** | [Development Guide](DEVELOPMENT/GUIDE.md) |
| **Get help** | [Troubleshooting](TROUBLESHOOTING.md) or [FAQ](FAQ.md) |

---

## 📖 Recommended Reading Order

1. **[INDEX.md](INDEX.md)** - Get oriented
2. **[STATUS.md](STATUS.md)** - Understand current state
3. **[Installation Guide](GETTING_STARTED/INSTALLATION.md)** - Get running
4. **[Architecture](ARCHITECTURE/OVERVIEW.md)** - Understand system design

---

**Questions?** Check [Troubleshooting](TROUBLESHOOTING.md) or [FAQ](FAQ.md)

**For organized navigation, start with [INDEX.md](INDEX.md)**
