 ...
 * Browser-safe: Falls back to console when running in browser
 */

// Check if we're in a browser environment
const isBrowser = typeof window !== 'undefined';

// Browser-safe logger that uses console
...
// Server-side winston logger - lazy initialization
let winstonLogger: any = null;
let loggerInstance: any = null;

// Lazy initialization function for server-side logger
const initializeServerLogger = () => {
  ...
  // Dynamic require to avoid bundling winston in client
  ...
  const sanitizeAndLog = (level: string, message: string, ...args: any[]) => {
    const sanitizedArgs = args.map(arg => sanitizeData(arg));
    winstonLogger.log(level, message, ...sanitizedArgs);
  };
...
// Export logger - browser-safe version or winston version (lazy initialized)
export const log = isBrowser ? createBrowserLogger() : (() => {
  if (!loggerInstance) {
    loggerInstance = initializeServerLogger();
  }
  return loggerInstance;
})();

// Export winston logger for advanced usage (server-side only)
export default isBrowser ? null : winstonLogger;
