...
export async function refreshTokenIfNeeded(): Promise<string | null> {
  try {
    ...
    try {
      ...
      if (tokenParts.length === 3) {
        ...
        if (exp) {
          ...
          if (timeUntilExpiry < FIVE_MINUTES && refreshToken) {
            log.debug('[token-refresh] Token expiring soon, refreshing...');
            try {
              ...
              log.warn('[token-refresh] Failed to refresh token:', refreshError.message);
      ...
      log.warn('[token-refresh] Could not decode token to check expiration');
    }

    return accessToken;
  } catch (error: any) {
    log.error('[token-refresh] Error checking token refresh:', error);
    return null;
  }
}
