-- ============================================================================
-- DocHub Database Schema Export Script
-- ============================================================================
-- INSTRUCTIONS:
-- 1. Open Supabase Dashboard → SQL Editor
-- 2. Paste this entire script
-- 3. Click "Run" (or press F5)
-- 4. Copy the output from the "output" column
-- 5. Save to: supabase/database_dump.sql
-- ============================================================================
...
SELECT 
    ...
    ' PRIMARY KEY (' || string_agg(quote_ident(a.attname), ', ' ORDER BY array_position(c.conkey, a.attnum)) || ');' as output
...
SELECT 
    ...
    ' REFERENCES ' || quote_ident(rt.relname) ||
...
WHERE c.contype = 'f'
  AND n.nspname = 'public'
GROUP BY t.relname, c.conname, rt.relname, c.confdeltype
ORDER BY t.relname, c.conname;
...
SELECT 
    ...
    ' EXECUTE FUNCTION ' || quote_ident(action_statement) || '();' as output
...
SELECT 
    pg_get_functiondef(p.oid) || ';' as output
FROM pg_proc p
...
SELECT 
    ...
    ' ENABLE ROW LEVEL SECURITY;' as output
FROM pg_tables
WHERE schemaname = 'public'
  AND EXISTS (
